...
<html lang="en">
    ...
    <body>
            ...
            <tbody>
                <tr>
                    <td class="border-0 pl-0" width="70%">
                        <h2 class="text-uppercase"><i><strong>{{ $invoice->name }}</strong></i></h2>
                    </td>
                    <td class="border-0 pl-0">
                        <h3>{{ __('invoices::invoice.serial') }}: <strong>{{ $invoice->getSerialNumber() }}</strong></h3>
                        <h3>{{ __('invoices::invoice.date') }}: <strong>{{ $invoice->getDate() }}</strong></h3>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <h3>
            {{ trans('invoices::invoice.amount_in_words') }}:<strong class="text-uppercase"> {{ $invoice->getTotalAmountInWords() }}</strong>
        </h3>
        <h3>
        ...
        @if($invoice->notes)
            <strong>{{ trans('invoices::invoice.notes') }}:</strong> <p> {!! $invoice->notes !!} </p>
        @endif

        <div style="position: absolute; bottom: 0; ">
            <p class="text-center" style="border-top: 2px solid #dee2e6; width: 100%;">
                @if($invoice->seller->address)
                    {{__('invoices::invoice.seller')  }} | {{  $invoice->seller->address  }}. 
                @endif   
                @if($invoice->seller->code)
                | {{  __('invoices::invoice.code') }}: {{ $invoice->seller->code  }}  |  
                @endif
