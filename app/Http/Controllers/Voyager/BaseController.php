        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($search->value != '' && $search->key && $search->filter) {
                ...
                if (in_array('relationship', $searchKeyType)) {
                    $infoAboutColumn = $dataRow->where('type', '=', 'relationship')
                        ->where('display_name', '=', $searchNames[$search->key])
                        ->first();
                ...
                } elseif (in_array('select_dropdown', $searchKeyType)) {
                    $infoAboutColumn = $dataRow->where('type', '=', 'select_dropdown')
                        ->where('display_name', '=', $searchNames[$search->key])
                        ->first();
                    if (property_exists($infoAboutColumn->details, 'options')) {
                        $columnOptions = json_decode(json_encode($infoAboutColumn->details->options), true);
                        $columnOptionsKeys = array_keys($columnOptions);
                        $i = 0;
                        foreach ($columnOptions as $optionKey => $optionValue) {
                            if (Str::contains($optionValue, $search->value) || Str::contains($optionValue, ucwords($search->value))) {
                                $optionKeys[$i] = $optionKey;
                                $i = $i + 1;
                            }
                        }
                        $query->whereIn($search->key, $optionKeys);
                    }

                } elseif (in_array('checkbox', $searchKeyType)) {
                    $infoAboutColumn = $dataRow->where('type', '=', 'checkbox')
                        ->where('display_name', '=', $searchNames[$search->key])
                        ->first();

                    if (property_exists($infoAboutColumn->details, 'on') && property_exists($infoAboutColumn->details, 'off')) {
                        $columnON = $infoAboutColumn->details->on;
                        $columnOFF = $infoAboutColumn->details->off;
                        if (Str::contains($columnON, $search->value) || Str::contains($columnON, ucwords($search->value))) {
                            $query->where($search->key, '=', 1);
                        }
                        if (Str::contains($columnOFF, $search->value) || Str::contains($columnOFF, ucwords($search->value))) {
                            $query->where($search->key, '=', 0);
                        }
                    }
                } else {
